  ...
  public D readRecordImpl(D reuse) throws DataRecordException, IOException {
    while (!allPartitionsFinished()) {
      ...
      while (!currentPartitionFinished()) {
        ++CCCt;
        System.out.println("currentPartitionFinished CCCt: " + CCCt);
        if (!this.messageIterator.hasNext()) {
          break;
        }

        MessageAndOffset nextValidMessage = this.messageIterator.next();
        System.out.println("nextValidMessage:  " + nextValidMessage);
        ...
        if (nextValidMessage.offset() < this.nextWatermark.get(this.currentPartitionIdx)) {
          System.out.println("nextValidMessage.offset(): " + nextValidMessage.offset());
          System.out.println("this.nextWatermark.get(this.currentPartitionIdx): " + this.nextWatermark.get(this.currentPartitionIdx));
...
//          byte[] rec = (byte[])record;
  ...
  private boolean allPartitionsFinished() {
    System.out.println("this.currentPartitionIdx:  " + this.currentPartitionIdx + "\n this.highWatermark.size(): " + this.highWatermark.size());
  ...
  private boolean currentPartitionFinished() {
    ...
    System.out.println("this.nextWatermark.get(this.currentPartitionIdx): " + this.nextWatermark.get(this.currentPartitionIdx));
    System.out.println("this.highWatermark.get(this.currentPartitionIdx): " + this.highWatermark.get(this.currentPartitionIdx));
    System.out.println(this.nextWatermark.toJson().toString());
    System.out.println(this.highWatermark.toJson().toString());
