...
public class KafkaWrapper implements Closeable {
  ...
  private class KafkaOldAPI extends KafkaAPI {
    ...
    public List<KafkaTopic> getFilteredTopics(List<Pattern> blacklist, List<Pattern> whitelist) {
      ...
      System.out.println("filteredTopics: " + filteredTopics);
    ...
    private List<TopicMetadata> fetchTopicMetadataFromBroker(String broker, List<Pattern> blacklist,
      ...
      System.out.println("topicMetadataList" + topicMetadataList.size());
      System.out.println("topicMetadataList blackList" + blacklist);
      System.out.println("topicMetadataList whiteList" + whitelist);
      List<TopicMetadata> filteredTopicMetadataList = Lists.newArrayList();
      for (TopicMetadata topicMetadata : topicMetadataList) {
      ...
      System.out.println("filteredTopicMetadataList" + filteredTopicMetadataList);
      return filteredTopicMetadataList;
    }
    ...
    private List<TopicMetadata> fetchTopicMetadataFromBroker(String broker, String... selectedTopics) {
      ...
      try {
        ...
        System.out.println("consumer :   " + (consumer != null));
        for (int i = 0; i < NUM_TRIES_FETCH_TOPIC; i++) {
          try {
            List<TopicMetadata> topicMetadatas = consumer.send(new TopicMetadataRequest(Arrays.asList(selectedTopics))).topicsMetadata();
            System.out.println("SSSSSSSSSSSSSSSSSSSSSSSSSSSSSSize" + topicMetadatas.get(0).topic());
    ...
    private long getOffset(KafkaPartition partition,
      ...
      for (int i = 0; i < NUM_TRIES_FETCH_OFFSET; i++) {
        try {
          ...
          System.out.println("ooooooooooooooooooffsetResponse: " + offsetResponse);
          System.out.println("offsetResponse.offsets(partition.getTopicName(), partition.getId())[0] : " + offsetResponse.offsets(partition.getTopicName(), partition.getId())[0]);
          System.out.println("offsetResponse.offsets(partition.getTopicName(), partition.getId()) : " + Arrays.toString(offsetResponse.offsets(partition.getTopicName(), partition.getId())));
